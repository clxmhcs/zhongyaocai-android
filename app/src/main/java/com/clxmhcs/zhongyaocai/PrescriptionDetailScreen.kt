...
fun PrescriptionDetailScreen(prescription: Prescription, data: AppData, viewModel: MainViewModel) {
    ...
    val rawContent = remember(prescription.items) {
        ...
        LazyColumn(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
            item {
                Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                    ...
                    TextButton(onClick = ::copyContent) {
                        Icon(Icons.Default.ContentCopy, "复制")
                        Text("复制", fontSize = 12.sp, modifier = Modifier.padding(start = 4.dp))
                    }
                }
            }
            item {
                Column(
                    Modifier.fillMaxWidth().background(Color(0xFFF2F2F7), shape = androidx.compose.foundation.shape.RoundedCornerShape(12.dp)).padding(horizontal = 12.dp, vertical = 10.dp),
                    verticalArrangement = Arrangement.spacedBy(8.dp)
                ) {
                    Row {
                        Text("处方药材", fontSize = 14.sp, fontWeight = FontWeight.SemiBold, color = Color.Gray)
                        Text("（点击蓝色药材名称可查看药材详细资料）", fontSize = 12.sp, color = Color.Gray)
                    }
                    prescription.items.chunked(2).forEach { pair ->
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                            PrescriptionDetailLine(pair[0], Modifier.weight(1f))
                            if (pair.size > 1) PrescriptionDetailLine(pair[1], Modifier.weight(1f)) else Spacer(Modifier.weight(1f))
                        }
                    }
                }
            }
            item {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Switch(checked = important, onCheckedChange = { checked ->
                            important = checked
                            viewModel.savePrescription(prescription.copy(isImportant = checked))
                        }, modifier = Modifier.padding(start = 6.dp))
                    }
                    Text(
                        rawContent,
                        fontFamily = FontFamily.Monospace,
                        modifier = Modifier.fillMaxWidth().background(Color(0xFFF2F2F7), shape = androidx.compose.foundation.shape.RoundedCornerShape(8.dp)).padding(16.dp)
                    )
                }
            }
            item {
                Button(
                    ...
                    Icon(Icons.Default.ContentCopy, "一键复制")
                    Text("一键复制", modifier = Modifier.padding(start = 8.dp))
                }
            }
        }
        if (copySuccess) {
            Row(
                modifier = Modifier.align(Alignment.TopCenter).padding(top = 12.dp).background(Color(0xFFF4426D), shape = androidx.compose.foundation.shape.RoundedCornerShape(12.dp)).padding(horizontal = 14.dp, vertical = 10.dp),
